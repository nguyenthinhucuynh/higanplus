...
### 10.3. Vai trò thật sự của R và W
...
- Chúng hoạt động như teacher / auxiliary critic để đưa signal nội dung và phong cách.

Vì vậy:

- Train GAN chính với `pretrained_r` + `pretrained_w` vẫn được xem là quy trình chuẩn của repo/paper.
- Nếu muốn “full scratch tuyệt đối”, phải train cả `R` và `W` từ đầu trước.
...
Các hệ số này được tính bằng tỉ lệ giữa `std_grad_adv` và `std` của gradient từ từng auxiliary loss.
...
Đây là nguyên nhân chính của OOM trên Kaggle T4 khi batch lớn.
...
Khi chạy trên Kaggle T4 15 GB:

- `batch_size = 16` OOM ngay trong `contextual_loss`
- `batch_size = 8` vẫn OOM muộn hơn ở `PatchDiscriminator` do peak memory dao động theo độ dài word và fragmentation
...
- contextual loss và patch discriminator làm peak memory không đều theo iter
...
- Model chính hiện không sống độc lập; nó phụ thuộc mạnh vào `Recognizer` và `WriterIdentifier` pretrained để train ổn định.
