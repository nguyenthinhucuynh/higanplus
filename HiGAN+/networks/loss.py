...
def masked_style_consistency_loss(base_style, vertical_style, horizontal_style):
    """
    Consistency loss to ensure masked styles are consistent with base style.
    This encourages the masking heads to learn complementary style information.

    Args:
        base_style: [B, style_dim] base style vector
        vertical_style: [B, style_dim] vertical masked style vector
        horizontal_style: [B, style_dim] horizontal masked style vector
    Returns:
        loss: scalar consistency loss
    """
    # L2 distance between base and masked styles
    vertical_loss = torch.mean(torch.abs(base_style - vertical_style))
    horizontal_loss = torch.mean(torch.abs(base_style - horizontal_style))

    # Encourage diversity between vertical and horizontal styles
    diversity_loss = -torch.mean(torch.abs(vertical_style - horizontal_style))
