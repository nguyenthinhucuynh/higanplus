...
from networks.loss import recn_l1_loss, CXLoss, KLloss, masked_style_consistency_loss
...
class GlobalLocalAdversarialModel(AdversarialModel):
    ...
    def train(self):
        ...
        self.averager_meters = AverageMeterManager(['adv_loss', 'fake_disc_loss',
                                                    ...
                                                    'gp_wid', 'gp_recn', 'mask_consistency_loss'])
        device = self.device
        ...
        for epoch in range(epoch_done, self.opt.training.epochs):
            for i, batch in enumerate(self.train_loader):
                ...
                if iter_count % self.opt.training.num_critic_train == 0:
                    ...
                    ### Masked Style Consistency Loss ###
                    mask_consistency_loss = torch.FloatTensor([0.]).to(self.device)
                    if masked_styles is not None and hasattr(self.models.E, 'use_masking') and self.models.E.use_masking:
                        vertical_style, horizontal_style = masked_styles
                        # Use mu as base style for consistency
                        base_style = mu if not self.vae_mode else mu
                        mask_consistency_loss = masked_style_consistency_loss(base_style, vertical_style, horizontal_style)
                    ...
                    self.averager_meters.update('mask_consistency_loss', mask_consistency_loss.item())
                ...
                if iter_count % self.opt.training.print_iter_val == 0:
                    ...
                    info = "[%3d|%3d]-[%4d|%4d] G:%.4f G-p:%.4f D-fake:%.4f D-real:%.4f " \
                           ...
                           "Recn-z:%.4f Recn-c:%.4f Ctx:%.4f Kl:%.4f Mask:%.4f" \
                           % (epoch, self.opt.training.epochs,
                              ...
                              meter_vals['mask_consistency_loss'])
